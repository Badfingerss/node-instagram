...
class Instagram {
  ...
  _fetchFeed(numOfPost, endCursor, count, posts) {
    ...
    .then(output => {
      count.n++;
      posts = posts.concat(output.feed.media.nodes);

      if(count.n < count.total) {
        return this._fetchFeed(numOfPost,
          output.feed.media.page_info.end_cursor, count, posts);
      } else {
        return posts;
      }
    });
  ...
  fetchFeed(numOfPost, total) {
    ...
    return this._request(url, 'get', {})
      .then(output => {
        posts = posts.concat(output.feed.media.nodes);
        return output.feed.media.page_info.end_cursor;
      })
      .then(endCursor => {
        if (total === 1) {
          return posts;
