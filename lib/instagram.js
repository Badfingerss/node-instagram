...
class Instagram {
  constructor(param) {
    param = param || {};
    this.query_id = '17882195038051799';
    this.follow_query_id = '17874545323001329';
  ...
  init() {
    return this._request(this.base, 'get', {raw: true})
      ...
      .then(body => {
        try {
          let feedQuery = body.match(/"(\d+)"\S+"FEED_PAGE/);
          if (feedQuery) {
            this.query_id = feedQuery[1];
          }
        } catch (e) {}
        try {
          let followQuery = body.match(/"(\d+)"\S+"(\d+)"\S+"FOLLOW_LIST/);
          if (followQuery) {
            this.follow_query_id = followQuery[2];
          }
        } catch (e) {}
      });
  }
  ...
  _fetchFeed(numOfPost, endCursor, count, posts) {
    if (!this.query_id) {
      return Promise.reject('No query_id');
    }
  ...
  fetchFollowsById(targetUserId, numOfFollower, cursor) {
    let url = 'https://www.instagram.com/graphql/query/?' +
      `query_id=${this.follow_query_id}&id=${targetUserId}&`+
